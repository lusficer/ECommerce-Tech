...
export default function NewReleasesPage() {
  ...
  const handleLoadMore = async () => { setLoadingMore(true); const nextPage = page + 1; await fetchProducts(nextPage); setPage(nextPage); setLoadingMore(false); };
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      <div className="bg-slate-900 py-12 relative overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 relative z-10 flex flex-col items-center text-center">
          <div className="w-16 h-16 bg-blue-500 rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-blue-500/30 animate-pulse">
            <Sparkles className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tight mb-4">New Releases</h1>
          <p className="text-slate-400 font-medium max-w-lg">Discover the hottest new tech products just launched on the market.</p>
        </div>
      </div>
            ...
            return (
              <Link href={`/products/${product.productId}`} key={product.productId} className="group relative flex flex-col bg-white p-4 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-500 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300">
                <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-10 pointer-events-none">
                  <span className="bg-blue-500 text-white text-[11px] font-black px-3 py-1.5 rounded-br-xl rounded-tl-lg uppercase tracking-wide shadow-md">NEW</span>
